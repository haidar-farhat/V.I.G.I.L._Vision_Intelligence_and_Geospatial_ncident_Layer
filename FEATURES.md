...
| Register for people and vehicles: enrol, forget, retain, sight | `TESTED` | `sentinel.registry`, now created by migration 5 on every deployment (built from the module's own schema so a fresh and an upgraded database are identical) and reachable as `Store.register` over the store's own connection, inside its transactions. Enrolment demands an actor and a lawful basis; `forget` deletes and reports; `sweep_expired` honours pinning. **No production caller yet** — nothing enrols, and the retention job does not sweep it |
...
| Footprint and coverage reflect camera health | `TESTED` | A camera whose health is DARK or FAULTED is hatched on the plan view and excluded from coverage — a camera producing nothing covers nothing. Driven from `camera_health()`, never from `record.is_running`, the flag this exists to stop trusting |
...
| Footprint far edge: range clamp or horizon, drawn differently | `TESTED` | A range-clamped far edge is solid (a limit the operator chose); a horizon-limited one is dashed (a physical limit), with a legend line. Decided from the pose's own geometry, not guessed |
...
| Query layer over events and incidents | `TESTED` | `sentinel.search` — bound parameters only, real objects newest-first, a total beside the page. Now in front of an operator: the **Investigation** tab (`InvestigationPanel`) with camera, window, severity, zone and free-text filters, "showing N of M" from `Results.total` so a truncated page is never mistaken for everything, a sentence for an empty result rather than an empty list, and results that select on the bus |
